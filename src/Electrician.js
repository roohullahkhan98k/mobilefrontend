...
const Profile = ({ name, number, info, onPress }) => {
  return (
...
const Electrician = ({ navigation }) => {
  ...
  const [electricianProfiles, setElectricianProfiles] = useState([]);
        ...
        <Animated.ScrollView contentContainerStyle={styles.contentContainer} style={{ opacity: fadeInOut }}>
          {electricianProfiles.map((profile, index) => (
            <Profile
              ...
              onPress={() => handleCallElectrician(profile.contact)}
            />
          ))}
        </Animated.ScrollView>
        <Footer />
